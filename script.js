    ...
    if(board) {
        ...
        if(window.state.quests) {
            window.state.quests.forEach((q, idx) => {
                ...
                item.className = `master-card tier-${safeDifficulty}`;
...
function updateFinancialSummary() { 
    ...
    if(remainingEl) {
        remainingEl.textContent = `$${safeBalance.toFixed(2)}`;
        remainingEl.style.color = safeBalance < 0 ? "var(--danger)" : "var(--success)";
    }
}
